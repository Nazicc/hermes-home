...
- Run a local LLM / run inference locally
- Use llama.cpp or GGUF models
- Set up a local model server (OpenAI-compatible API)
- Quantize or convert models to GGUF format
- Run models on Apple Silicon (M1/M2/M3/M4) or GPU without CUDA dependencies
- Integrate local LLM inference into an application (CLI, Python, HTTP)

**NOT this skill when:**
- User needs cloud-hosted models (use `openai` or `huggingface-hub` for inference endpoints)
- User needs model training or fine-tuning (use `peft`, `trl-fine-tuning`, or `huggingface-hub`)
- User needs vLLM or FlashAttention-based inference (use `vllm` or `flash-attention`)
...
| Type | Bits | Size (7B) | Size (13B) | Quality | Use Case |
|------|------|-----------|------------|---------|----------|
| Q2_K | 2.5 | ~2.8 GB | ~5 GB | Low | Extreme compression (testing only) |
| Q3_K_S | 3.0 | ~3.0 GB | ~5.5 GB | Low-Med | Memory constrained |
| Q3_K_M | 3.3 | ~3.3 GB | ~6 GB | Medium | Fits small devices |
| Q4_K_S | 4.0 | ~3.8 GB | ~7 GB | Med-High | Speed critical |
| **Q4_K_M** | 4.5 | ~4.1 GB | ~7.5 GB | High | **Recommended default** |
| Q5_K_S | 5.0 | ~4.6 GB | ~8.4 GB | High | Quality focused |
| Q5_K_M | 5.5 | ~4.8 GB | ~8.7 GB | Very High | High quality |
| Q6_K | 6.0 | ~5.5 GB | ~10 GB | Excellent | Near-original |
| Q8_0 | 8.0 | ~7.2 GB | ~13 GB | Best | Maximum quality, minimal degradation |
...
- **CUDA/GQA mismatch**: "ggml_flash_attn: GGML_BLOCK_SIZE_16 not supported" — model was trained with a different GQA configuration. Try Q4_0 or Q8_0 instead of K-quants, or use a model with matching GQA.
- **Metal not working on Apple**: Ensure `-ngl 99` is set. If still slow, verify Metal is enabled in cmake: `GGML_METAL=ON`.
...
- **Context overflow**: Increase `--ctx-size` or `-c` flag (must be ≤ model's max context).
- **Model not loading**: Check that the GGUF file is complete and not corrupted. Re-download if needed.
...
- **Ollama model not found**: Ollama uses its own model format — GGUF models must be imported via Modelfile.
...
- Runs on CPU-only (no GPU required) for small models
- GGUF format enables easy model sharing and conversion
- K-quants reduce model size 4-8x with minimal quality loss
- OpenAI-compatible server enables drop-in replacement in existing applications
- Metal acceleration on Apple Silicon achieves near-CUDA performance
