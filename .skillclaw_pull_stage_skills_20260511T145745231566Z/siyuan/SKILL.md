---
name: siyuan
description: "SiYuan Note API for searching, reading, creating, and managing blocks and documents in a self-hosted knowledge base via curl. Use when: you need to interact with a self-hosted SiYuan knowledge base, manage blocks/documents programmatically, search notes, or automate knowledge base workflows. NOT for: cloud-based note apps (use Notion/Obsidian), real-time collaboration, or when the SiYuan desktop app is more convenient."
category: general
---
...
1. Install and run SiYuan (desktop, server, or Docker)
2. Get your API token and note the port:
   - **Desktop app**: Settings > About > API token, port `6806`
   - **Kernel/server**: Token stored in workspace, port `46312`
3. Store credentials in `~/.hermes/.env`:
   
   SIYUAN_TOKEN=your_token_here
   SIYUAN_URL=http://127.0.0.1:6806
   
   `SIYUAN_URL` defaults to `http://127.0.0.1:6806` if not set. For kernel deployments, use `http://127.0.0.1:46312`.

## API Basics

All SiYuan API calls are **POST with JSON body** (except where noted). Every request follows this pattern:
...
| Create block | `/api/block/createBlock` |
...
bash
curl -s -X POST "${SIYUAN_URL:-http://127.0.0.1:6806}/api/search/searchBlock" \
  -H "Authorization: Token $SIYUAN_TOKEN" \
  -H "Content-Type: application/json" \
  -d '{"k": "search term", "rows": 20, "types": ["block", "document"]}' | jq '.data'
...
bash
curl -s -X POST "${SIYUAN_URL:-http://127.0.0.1:6806}/api/block/getBlockByID" \
  -H "Authorization: Token $SIYUAN_TOKEN" \
  -H "Content-Type: application/json" \
...
1. **Automated note creation**: Create notes from external data sources
2. **Knowledge base sync**: Sync external databases into SiYuan
3. **Batch operations**: Update or delete multiple blocks programmatically
4. **Search integration**: Search SiYuan notes from other tools
5. **Backup/restore**: Extract and re-import data

## Pitfalls

- **All endpoints are POST** -- even read-only operations. Do not use GET unless specified (e.g., `lsNotebooks` in some configurations).
...
- **Port differences**: Desktop app uses port 6806, kernel/server uses port 46312. Use the correct port for your deployment.
