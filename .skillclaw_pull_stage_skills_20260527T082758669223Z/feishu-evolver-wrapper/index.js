        ...
        while (attempts < MAX_RETRIES && !ok) {
            ...
            try {
                ...
                try {
                    ...
                    // Send merged report (single card with summary)
                    var gitSuffix = '';
                    if (gitInfo && gitInfo.shortHash) {
                        gitSuffix = '\n\nGit: ' + gitInfo.commitMsg + ' (' + gitInfo.shortHash + ')';
                    }

                    const effectiveLang = REPORT_LANG === 'auto'
                        ? (FEISHU_CN_REPORT_GROUP ? 'cn' : 'en')
                        : REPORT_LANG;

                    const blastRadius = latestEvent && latestEvent.blast_radius ? latestEvent.blast_radius : null;
                    const summaryData = {
                        success: true,
                        duration,
                        filesChanged: blastRadius ? blastRadius.files : undefined,
                        linesAdded: blastRadius ? blastRadius.lines : undefined,
                        infoCount: sessionLogs.infoCount,
                        errorCount: sessionLogs.errorCount,
                        errors: sessionLogs.errors.slice(-3)
                    };

                    if (effectiveLang === 'cn' || effectiveLang === 'both') {
                        try {
                            await sendReport({
                                cycle: cycleTag,
                                title: `进化 #${cycleTag}`,
                                status: zhStatus + gitSuffix,
                                target: FEISHU_CN_REPORT_GROUP || undefined,
                                lang: 'cn',
                                summary: summaryData
                            });
                        } catch (reportErr) {
                    ...
                    if (effectiveLang === 'en' || effectiveLang === 'both') {
                        try {
                            await sendReport({
                                cycle: cycleTag,
                ...
                if (attempts >= MAX_RETRIES) {
                    try {
                        ...
                        const failBlast = (function() {
                            try { var evt = readLatestEvolutionEvent(); return evt && evt.blast_radius ? evt.blast_radius : null; } catch(_) { return null; }
                        })();
                        const failSummary = {
                            success: false,
                            duration,
                            filesChanged: failBlast ? failBlast.files : undefined,
                            linesAdded: failBlast ? failBlast.lines : undefined,
                            infoCount: sessionLogs.infoCount,
                            errorCount: sessionLogs.errorCount,
                            errors: sessionLogs.errors.slice(-3)
                        };
