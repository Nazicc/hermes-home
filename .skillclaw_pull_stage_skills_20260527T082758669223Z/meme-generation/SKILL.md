...
description: "Generate real meme images by picking a template and overlaying text with Pillow. Produces actual .png meme files. Use when: user wants to make/create/generate a meme about a specific topic, situation, or frustration. NOT for: generating video memes, GIFs, creating original artwork from scratch, or when photographic-quality image generation is required."
category: general
---
...
### Additional Known Templates

Surprised Pikachu, Success Kid, Bad Luck Brian, Y U No?, Futurama Fry, Star Wars Yoda, Change My Mind

### Dynamic Templates (from imgflip API)

Any template not in the curated list can be used by name or imgflip ID. Search with:
bash
...
5. **Verify with vision** (if `vision_analyze` is available): Check the result looks good:
   ...
   If the vision model flags issues, try the other mode or regenerate the scene.
6. Return the image with `MEDIA:/tmp/meme.png`

## Text Styling

- Font: Helvetica Bold (falls back to default Pillow font)
- Text color: White with black outline (for contrast)
- Text size: Auto-scaled based on image dimensions
- All caps applied automatically for standard meme aesthetic

## Examples

**"debugging production at 2 AM":**
bash
python "$SKILL_DIR/scripts/generate_meme.py" this-is-fine /tmp/meme.png "SERVERS ARE ON FIRE" "This is fine"


**"choosing between sleep and one more episode":**
bash
python "$SKILL_DIR/scripts/generate_meme.py" drake /tmp/meme.png "Getting 8 hours of sleep" "One more episode at 3 AM"


**"the stages of a Monday morning":**
bash
python "$SKILL_DIR/scripts/generate_meme.py" expanding-brain /tmp/meme.png "Setting an alarm" "Setting 5 alarms" "Sleeping through all alarms" "Working from bed"


## Listing Templates

bash
python "$SKILL_DIR/scripts/generate_meme.py" --list
