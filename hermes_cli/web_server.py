...
# Prefixes that are always public (support path variables like /api/pool/{provider}/...)
_PUBLIC_API_PREFIXES: tuple = (
    "/api/pool/",
)
...
# ---------------------------------------------------------------------------
# Credential pool API — used by SkillClaw relay for multi-key rotation.
# Returns masked key info (never exposes full key to caller).
# Protected by localhost-only binding; not added to _PUBLIC_API_PATHS
# to avoid accidental exposure on non-localhost binds.
# ---------------------------------------------------------------------------

class _PoolReportRequest(BaseModel):
    key_id: str
    success: bool


@app.get("/api/pool/{provider}/next_key")
async def pool_next_key(provider: str):
    """Select and return the next credential from the pool.

    Response shape:
        key_id        pool entry id (stable identifier)
        api_key       full API key (Bearer token)
        token_prefix  last 8 chars of the key for logging/display
    """
    pool = load_pool(provider)
    if pool is None:
        raise HTTPException(status_code=404, detail=f"No credential pool for provider: {provider}")
    entry = pool.select()
    if entry is None:
        raise HTTPException(status_code=503, detail=f"No available credentials in pool: {provider}")
    return {
        "key_id": entry.id,
        "api_key": entry.access_token,
        "token_prefix": entry.access_token[-8:],
    }


@app.post("/api/pool/{provider}/report")
async def pool_report(provider: str, body: _PoolReportRequest):
    """Report success/failure for a credential, updating request counts.

    On failure, marks the credential as exhausted and triggers rotation.
    """
    pool = load_pool(provider)
    if pool is None:
        raise HTTPException(status_code=404, detail=f"No credential pool for provider: {provider}")
    # Find entry by id
    entry = next((e for e in pool._entries if e.id == body.key_id), None)
    if entry is None:
        raise HTTPException(status_code=404, detail=f"Key id not found in pool: {body.key_id}")
    if body.success:
        with pool._lock:
            import dataclasses
            updated = dataclasses.replace(entry, request_count=(entry.request_count or 0) + 1)
            updated.request_count = (entry.request_count or 0) + 1
            pool._replace_entry(entry, updated)
            pool._persist()
        return {"ok": True, "key_id": body.key_id, "request_count": updated.request_count}
    else:
        # Mark exhausted and rotate to next
        next_entry = pool.mark_exhausted_and_rotate(
            status_code=429,
            error_context={"reported_by": "skillclaw_relay"},
        )
        return {
            "ok": True,
            "key_id": body.key_id,
            "rotated": next_entry.id if next_entry else None,
        }
